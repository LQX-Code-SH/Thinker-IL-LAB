# Walker S2 EDU 探索者 仿真全流程：采集 -> 转换 -> 训练 -> 评估 -> 部署
...
转换脚本：`ubt_IL/scripts/convert/walker_s2/convert.sh`，转换配置文件路径：在`/ubt_IL/scripts/convert/walker_s2/configs/`，可根据训练需求选择/修改配置文件。
