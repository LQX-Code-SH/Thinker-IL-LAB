# Walker S2 EDU 探索者：真机控制 SDK（robot_control）

> `ubt_IL/walker/walker_sdk_ros2/robot_control/`：Walker S2 EDU 探索者 真机 Python 控制包，提供 Python API 直接控制真机的身体 17 DOF 与 V4 手 14 DOF。真机部署脚本 `robot_ready.sh` 的初始化/回零（`--init` / `--home`）即由本包 `robot_control.py` 实现（见 [真机部署](deploy.md#初始化回零robot_readysh)）。

从仓库内 `executor_node_sdk.py`（`walker_sdk_ros2/../rosa_vla_additional/`）提取的纯机器人控制版本，**去除 VLA 推理依赖**，提供 Python API 直接控制真机。

控制方法：SDK 控制器（`RobotCommand`/`JointCmd`，`MODE_POSITION=2`），两层控制架构见下文。
