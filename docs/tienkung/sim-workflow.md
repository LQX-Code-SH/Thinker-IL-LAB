...
仿真 HDF5 数据由 **`ubt_sim`** 项目采集（详见 [仿真平台](sim-setup.md)）。要点：
...
脚本：`/ubt_IL/scripts/deploy/tienkung_pro/rollout.sh`，仿真部署使用ubt_sim模块代替机器人真机进行测试。该仿真环境与真机ROS话题部署和通信方法一致，可用于真机部署前的验证工作，避免真机动作错误造成损坏等严重后果。仿真模块容器独立运行，与模型训练推理容器在同一主机通过本地回环127.0.0.1网段进行ROS通信。仿真环境使用方法详见：[仿真平台](sim-setup.md)。
