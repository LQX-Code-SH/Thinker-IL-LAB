...
基于 LeRobot 的天工行者（Walker TienKung）与 Walker S2 EDU 探索者机器人模仿学习训练与部署平台。
...
```
TienKung:
LeRobot (3.12) ──ZMQ 5559/5560──► Bridge2 (3.10) ──ROS2 DDS──► 天工行者硬件
                │
                └──ZMQ 5558──► ImageServer (独立进程) ──► 相机硬件

Walker S2 EDU:
LeRobot (3.12) ──ZMQ 5561/5562──► Walker Bridge2 (3.10) ──ROS2 DDS──► Walker S2 EDU
                │
...
| `tienkung/` | 天工行者插件 | 容器内 | 独立 pip 包，注册为 `tienkung` 类型 |
| `walker/` | Walker S2 EDU 插件 + Bridge + ROS2 SDK | 容器内 | 独立 pip 包，注册为 `walker` / `walker_camera` 类型 |
...
| `scripts/eval/` | 离线策略评估（机器人无关，天工行者/Walker S2 EDU 共用） | 容器内 | eval_policy.py |
...
| `/ubt_IL/tienkung/` | 天工行者插件（bind mount） |
| `/ubt_IL/walker/` | Walker S2 EDU 插件、Bridge2、ROS2 SDK/messages（bind mount） |
...
## 31-Dim Walker S2 EDU State/Action Vector

Walker S2 EDU 真机插件默认 31 维：
...
- **天工行者插件与 LeRobot 源码相互独立**：插件通过 `@register_subclass` 注册，无需修改上游代码
