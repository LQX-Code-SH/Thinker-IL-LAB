...
class Utars_1RGBDataConfig(BaseDataConfig):
    ...
    language_keys = ["annotation.human.action.task_description"]
    observation_indices = [0]
    action_indices = list(range(10))  # 改为只采样当前时间步
    def modality_config(self) -> dict[str, ModalityConfig]:
        video_modality = ModalityConfig(
            delta_indices=self.observation_indices,
            modality_keys=self.video_keys,
...
class Utars_1RGB_From_TeleopDataConfig(BaseDataConfig):
    video_keys = ["video.rgb"]
    state_keys = [
        "state.left_arm",
        "state.right_arm",
...
DATA_CONFIG_MAP = {
    ...
    "utars1": Utars_1RGBDataConfig(),
    "utars1_tele": Utars_1RGB_From_TeleopDataConfig(),
}
