...
本文档给出 Walker S2 从克隆代码到真机/仿真器部署的最简路径，按需执行每一步。各步骤的完整参数说明与可复制命令见工作流专项文档：
...
python hf_manager.py pull qingxiangliu/Wlaker_Pick_part_real_10d_2RGB   # 下载 Walker S2 抓取测试数据集
python hf_manager.py pull qingxiangliu/Walker_S2_sim_10_2RGB  # 下载 Walker S2 仿真抓取测试数据集
```
...
脚本：`/ubt_IL/scripts/eval/eval_policy.py`（与天工共用），在数据集上离线推理，对比预测动作 vs 真值动作的 MSE，部署前先量化策略质量。完整流程与参数说明见：
