...
# 控制模式额外需要（walker 需要 SDK 消息包，天工行者无疆仅需 ROS2）：
...
> ⚠️ **本容器内请用 web viewer（浏览器）**：Rerun 原生 viewer 是 wgpu/egui 应用，
> 容器里只有 NVIDIA vendor GL（无 Mesa、无 vulkan surface），wgpu 拿不到 adapter
> 会静默启动失败——脚本检测不到 viewer 进程时会打印警告并给出本指引。
> 本地弹窗模式（`--mode preview`）仅在有正常 GL/Vulkan 窗口环境的主机上可用。
...
照它打开即可。⚠️ 末尾 `?url=` 参数是数据源地址，**不能省**——只打开
`http://localhost:9090` 会停在 Rerun 欢迎页看不到数据（rerun 0.23 的
`serve_web_viewer` 不会自动注入数据源）。远程浏览器需同时转发 9090 与 9876
两个端口（VSCode PORTS 面板 / `ssh -L 9090:localhost:9090 -L 9876:localhost:9876`）。
...
- 天工行者无疆 同理，脚本换成 `playback_tienkung_pro.py`

- `--episode` 缺省 = `dataset/<robot>/` 下最新（时间戳目录名最大）episode
- `--max-frames N` 只记录前 N 帧（大文件调试用）
- Rerun 面板：`cameras/color/<相机名>`（图像）、`cameras/depth/...`（天工行者无疆深度）、
...
`/mc/sdk/robot_command` + 双手 `JointCommand(mode=5)` + 双夹爪 `GripCmd`；天工行者无疆
...
| `--align-first` | 关 | 播放前平滑移动到首帧位姿（walker: quintic 插值 3s；天工行者无疆: 关节空间 ramp ~2s） |
| `--dry-run` | 关 | 只打印不发布 |
| `--interp` | 关 | 【walker】记录帧间线性插值到 `--rate`（更平滑，偏离原始记录） |
| `--spd/--cur` | `0.2/5.0` | 【天工行者无疆】电机默认速度/电流 |
| `--arm-clamp-lo/hi` | `±2.5` | 【天工行者无疆】臂限位（teleoperation 侧无臂限位常量，保守假设） |
