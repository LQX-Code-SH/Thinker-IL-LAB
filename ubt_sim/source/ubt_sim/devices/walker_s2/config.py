...
WALKER_S2_GRIPPER_HOME_POSE = {
    # 与 _grip_cmd_to_joint_targets 的 home 映射对齐：HOME_OPENING_M == OPENING_MAX_M
    # 即全开，对应仿真关节 = SIGN * (-CLOSING)。旧公式 (MAX-HOME_OPENING)/MAX*CLOSING
    # 因 HOME_OPENING==MAX 退化为 0（半开），启动后首条 home 指令会从 0 跳变到全开。
    name: WALKER_S2_GRIPPER_JOINT_SIGNS[name] * (-WALKER_S2_GRIPPER_JOINT_CLOSING_M)
